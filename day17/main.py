...
def dijkstra(cells, start_vertex, size):
    ...
    while not pq.empty():
        ...
        for new_dir, cell in current_vertex.neighbours.items():
            ...
            distance = cell.loss
            if (cell,new_dir,dir_counter) not in visited:
                old_cost = D[cell.x, cell.y, dir, dir_counter]
                new_cost = dist + distance
                if new_cost < old_cost:
                    #if (cell.x, cell.y) == (2,1):
                        #pass
                    pq.put((new_cost, cell, new_dir, dir_counter))
                    D[cell.x, cell.y, new_dir, dir_counter] = new_cost
                    print(f"{cell.x} {cell.y} | {new_cost} | {new_dir} {dir_counter}")
